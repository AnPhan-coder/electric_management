...
public interface HoaDonReponsitory extends JpaRepository<HoaDon, String> {
    ...
    @Query("SELECT DISTINCT new backend.elecmanagement.dto.request.CTHOADONNO(" +
            "h.mahd, h.ky, h.chisodau, h.chisocuoi,h.ngaylaphd, h.tongthanhtien, k.tenkh, k.diachi,k.dt, d.madk) " +
            "FROM HoaDon h " +
            "JOIN CtHoaDon c ON h.mahd = c.id.mahd " +
            "JOIN DienKe d ON c.id.madk = d.madk " +
            "JOIN KhachHang k ON d.makh = k.makh " +
            "WHERE h.tinhtrang = false")
    List<CTHOADONNO> findDanhSachNo();
    @Query("SELECT DISTINCT new backend.elecmanagement.dto.request.CTHOADONNO(" +
            "h.mahd, h.ky, h.chisodau, h.chisocuoi,h.ngaylaphd, h.tongthanhtien, k.tenkh, k.diachi,k.dt, d.madk) " +
            "FROM HoaDon h " +
            "JOIN CtHoaDon c ON h.mahd = c.id.mahd " +
